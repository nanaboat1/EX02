...
int main( ) {
   ...
   //Simple Testing
    if ( isTriangle(Triangle(3,2,4)) != Scalene) {
        std::cout << "failed for <3,2,4>\n" << std::endl;
    }  

    if ( isTriangle(Triangle(5,4,3)) != Right) {
        std::cout << "failed for <5,4,3>\n" << std::endl;
    } 

    if ( isTriangle(Triangle(1,1,1)) != Equilateral) {
        std::cout << "failed for <1,1,1>\n" << std::endl;
    }

    if ( isTriangle(Triangle(-1,-1,-2)) != NotATriangle) {
        std::cout << "failed for <-1,-1,-2>\n" << std::endl;
    }
    ...
    if ( isTriangle(Triangle(4,4,6)) != Isosceles) {
        std::cout << "failed for <4,4,6>\n" << std::endl;
    }

    if ( isTriangle(Triangle(INT_MAX,INT_MAX,INT_MAX)) != NotATriangle) {
        std::cout << "failed for <-1,-1,-2>\n" << std::endl;
    }
    ...
    for ( int i=0; i<5; i++ ) {
        test_run.push_back(triangleTest(Triangle(x,y,z), Scalene));
    }
